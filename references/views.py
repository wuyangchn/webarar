...
def journal_ranking(request):
    ...
    JIF21_list = file_funcs.open_xls(os.path.join(settings.STATICFILES_DIRS[0], f'document/JCR_JIF_2021.xls'))
    JIF21_list = JIF21_list['JCR_JIF_2021'][1:]
    JIF21_dict = {}
    for journal in JIF21_list:
        JIF21_dict.update({str(journal[0]).upper(): journal})
    JIF22_list = file_funcs.open_xls(os.path.join(settings.STATICFILES_DIRS[0], f'document/JCR_JIF_2022.xls'))
    ...
    def add_data(tag):
        ...
        for home, dirs, files in os.walk(file_url):
            for file in files:
                ...
                content = file_funcs.open_xls(file_path)
